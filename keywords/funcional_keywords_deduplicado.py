...
def build_master_raw(excel_data: pd.ExcelFile) -> pd.DataFrame:
    ...
    cust_df["Comp Click Share"] = pd.Series([np.nan] * len(df_cust))
    cust_df["Niche Click Share"] = pd.Series([np.nan] * len(df_cust))
    cust_df["Comp Depth"] = pd.Series([np.nan] * len(df_cust))
    cust_df["Niche Depth"] = pd.Series([np.nan] * len(df_cust))
    cust_df["Relevancy"] = pd.Series([np.nan] * len(df_cust))
    ...
    comp_df["ASIN Click Share"] = pd.Series([np.nan] * len(df_comp))
    comp_df["Comp Click Share"] = df_comp.iloc[:, 2]
    comp_df["Niche Click Share"] = pd.Series([np.nan] * len(df_comp))
    comp_df["Comp Depth"] = df_comp.iloc[:, 5]
    comp_df["Niche Depth"] = pd.Series([np.nan] * len(df_comp))
    comp_df["Relevancy"] = pd.Series([np.nan] * len(df_comp))
    comp_df["Fuente"] = "CompKW"
    comp_df["ABA Rank"] = pd.Series([np.nan] * len(df_comp))

    mining_df = pd.DataFrame()
    mining_df["Search Terms"] = df_mining.iloc[:, 0]
    mining_df["Search Volume"] = df_mining.iloc[:, 5]
    mining_df["ASIN Click Share"] = pd.Series([np.nan] * len(df_mining))
    mining_df["Comp Click Share"] = pd.Series([np.nan] * len(df_mining))
    mining_df["Niche Click Share"] = df_mining.iloc[:, 15]
    mining_df["Comp Depth"] = pd.Series([np.nan] * len(df_mining))
